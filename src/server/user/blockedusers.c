...
void freeBlockedUsersNode(BUNode *node);

BUList *createBUList() {
    BUList *list = Malloc(sizeof(*list));
    list->head = NULL;
...
void freeBlockedUsers(BUList *list) {
    freeBlockedUsersNode(list->head);
    Pthread_rwlock_destroy(&list->rwlock); // did I read something about resetting attributes to null before destroying?
}

// DON'T FREE USER
void freeBlockedUsersNode(BUNode *node) { // 'block' name
    if (node != NULL) {
        freeBlockedUsersNode(node->next);
        free(node);
    }
}
